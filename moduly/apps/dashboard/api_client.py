...
def get_system_scheduler_health(access_token: str) -> dict[str, object]:
    response = _request(
        "GET",
...
def get_system_database_health(access_token: str) -> dict[str, object]:
    response = _request(
        "GET",
...
def get_kalorimetry_devices(access_token: str, limit: int = 5000) -> list[str]:
    response = _request(
        "GET",
        "/api/v1/kalorimetry/devices",
        access_token=access_token,
        query_params={"limit": limit},
    )
    payload = response.json()
...
def list_kalorimetry_outlier_reviews(
    access_token: str,
    *,
    review_status: str | None = "PENDING",
    identifikace: str | None = None,
    ...
    response = _request(
        "GET",
        "/api/v1/kalorimetry/outlier-reviews",
        access_token=access_token,
        query_params=query_params,
    )
    payload = response.json()
...
def update_kalorimetry_outlier_review(
    access_token: str,
    review_id: int,
    payload: dict[str, object],
) -> dict[str, object]:
    response = _request(
        "PATCH",
        f"/api/v1/kalorimetry/outlier-reviews/{review_id}",
        access_token=access_token,
        json_payload=payload,
    )
    return dict(response.json())
