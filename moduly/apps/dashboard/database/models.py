...
class Dashboard_MapLayer(Base):
    ...
    map_labels_default_visible: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True, server_default="true")
    sync_mistnosti_filters: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, server_default="false")
    ...
    def get_map_label_columns(self) -> list[str]:
        return self._load_json_list(self.map_label_columns)

    def set_map_label_columns(self, columns: list[str]) -> None:
        self.map_label_columns = json.dumps(columns, ensure_ascii=True)
