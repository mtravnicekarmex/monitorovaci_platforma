...
def prepare_consumption_history(df: pd.DataFrame) -> pd.DataFrame:
    ...
    history_df = history_df.dropna(subset=["chart_time", "objem"]).sort_values("chart_time").reset_index(drop=True)
    if history_df.empty:
        return history_df
...
def render_average_consumption_section(history_df: pd.DataFrame) -> None:
    with st.container(border=True):
        ...
        if history_df.empty:
            st.info("Pro tento plynoměr zatím není dostatek dat pro výpočet průměrné spotřeby.")
            return
...
def build_daily_chart(
    ...
    if prediction_df is not None and not prediction_df.empty:
        ...
        day_indexes = {
            ...
            for row in chart_df[["date", "day_index"]].itertuples(
                index=False
            )
        }
...
def render_prediction_status(prediction: dict[str, object]) -> None:
    ...
    with st.container(border=True):
        st.subheader("Predikce spotřeby")
        ...
        expected_total = round(
            float(rows["ocekavana_spotreba"].sum()),
            3,
        )
        st.metric(
...
def render_dashboard() -> None:
    ...
    top_cols[3].metric("Poslední odečet", latest_volume)
            ...
            with legend_col:
                st.markdown(
                    ...
                    <div style="text-align:right; font-size:12px; color:#6b7280; line-height:1.4; padding-top:0.35rem;">
                        ...
                        <span style="display:inline-flex; align-items:center; gap:0.35rem; margin-left:0.9rem;">
                            <span style="display:inline-block; width:16px; border-top:2px dashed #dc2626;"></span>
                            Predikce
                        </span>
                    </div>
                    """,
            ...
            if history_df.empty:
                ...
                if not monthly_prediction_df.empty:
                    ...
                    if not prediction_monthly.empty:
                        ...
                        monthly_chart = monthly_chart + prediction_line
                st.altair_chart(
        ...
        with st.container(border=True):
            ...
            if history_df.empty:
                ...
                if daily_history.empty:
                    ...
                    seven_day_start = (
                        pd.Timestamp(daily_history["date"].min()).normalize()
                    )
