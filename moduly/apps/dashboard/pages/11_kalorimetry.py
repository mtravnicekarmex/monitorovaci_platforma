...
DATE_RANGE_KEY = "kalorimetry_overview_date_range"
DETAIL_KEY = "kalorimetry_overview_detail"
GRAPH_KEY = "kalorimetry_overview_graph"
APPLIED_KEY = "kalorimetry_overview_applied"

DETAIL_OPTIONS = ("Ne", "Měsíčně", "Denně", "Hodinově")
GRAPH_OPTIONS = ("Ne", "Ano")
ENERGY_STATE_COLOR = "#f97316"
ENERGY_CONSUMPTION_COLOR = "#dc2626"
PREDICTION_COLOR = "#dedcd9"


st.set_page_config(
    page_title="Kalorimetry - Přehled",
    page_icon="♨️",
    layout="wide",
)


require_page_access("kalorimetry_overview")
...
def render_overview_sidebar(
    ...
    if not ident_options:
        st.warning("Pro aktuální kombinaci oprávnění nejsou k dispozici žádné kalorimetry.")
        st.stop()
    ...
    with st.sidebar:
        st.markdown("---")
        st.subheader("Filtry")
        with st.form("kalorimetry_overview_filters"):
            identifikace = st.selectbox("Kalorimetr", ident_options, key=DEVICE_KEY)
...
def prepare_measurements(df: pd.DataFrame) -> pd.DataFrame:
    ...
    prepared["spotreba_energie"] = pd.to_numeric(prepared["spotreba_energie"], errors="coerce")
    prepared["seriove_cislo"] = prepared["seriove_cislo"].astype(str)
    prepared["platne"] = prepared["platne"].fillna(True).astype(bool)
    prepared = prepared.dropna(subset=["chart_time", "spotreba_energie"]).sort_values("chart_time").reset_index(drop=True)
...
def build_boundary_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    boundary = boundary.drop_duplicates(subset=["date", "spotreba_energie", "seriove_cislo"])
    return boundary.rename(
        columns={
            "date": "Datum",
            "spotreba_energie": "Stav energie",
            "identifikace": "Kalorimetr",
            "seriove_cislo": "Sériové číslo",
            "platne": "Platné",
        }
    )[["Datum", "Stav energie", "Kalorimetr", "Sériové číslo", "Platné"]]
...
def build_detail_table(df: pd.DataFrame, detail_level: str) -> pd.DataFrame:
    ...
    resampled = (
        ...
        .agg(
            spotreba_energie=("spotreba_energie", "last"),
    ...
    resampled = resampled[resampled["spotreba_energie"].notna()].copy()
    if resampled.empty:
        return resampled
...
def render_overview_metrics(
    ...
    with st.container(key="mobile_metric_grid_kalorimetry_summary"):
        columns = st.columns(4)
        columns[0].metric(
            "Spotřeba energie",
            format_energy_metric(summary["actual_total"]),
        )
        if prediction_available:
            columns[1].metric(
                "Očekávaná spotřeba",
                format_energy_metric(summary["expected_total"]),
            )
            columns[2].metric(
                "Odchylka",
                format_energy_metric(summary["deviation"]),
            )
            columns[3].metric(
                "Odchylka [%]",
                (
                    "N/A"
                    if summary["deviation_pct"] is None
                    else f"{summary['deviation_pct']:+.1f} %"
                ),
            )
        else:
            columns[1].metric("Očekávaná spotřeba", "Nedostupné")
            columns[2].metric("Odchylka", "Nedostupné")
            columns[3].metric("Odchylka [%]", "Nedostupné")
    if prediction.get("availability_reason") == "insufficient_history":
        st.caption("Pro tento kalorimetr zatím není dostatečná historie.")
...
def render_prediction_summary(prediction: dict[str, object]) -> None:
    ...
    if not prediction.get("prediction_available") or rows.empty:
        st.metric("Očekávaná spotřeba", "Nedostupné")
        return
    st.metric(
        "Očekávaná spotřeba",
...
def build_prediction_chart(
    ...
    *,
    ...
    y_title: str = "Spotřeba energie",
    tooltip_title: str = "Očekávaná spotřeba",
) -> alt.Chart:
    return (
        alt.Chart(prediction_df.dropna(subset=["date", value_column]))
        .mark_line(color=PREDICTION_COLOR, strokeWidth=2.5)
        .encode(
...
def render_graph_legend(show_prediction: bool) -> None:
    items = [
        ...
        f'<span style="display:inline-block;width:0.85rem;height:0.85rem;border-radius:999px;background:{ENERGY_CONSUMPTION_COLOR};"></span>'
        "Spotřeba energie</span>"
    ]
    if show_prediction:
        items.append(
    ...
    st.markdown(
        f'<div style="margin-top:0.75rem;font-size:0.92rem;">{"".join(items)}</div>',
...
def render_graphs(
    ...
    if detail_level == "Ne" or detail_df.empty:
        chart_source_df = round_consumption_columns(df, columns=("spotreba_energie", "spotreba"))
        chart_cols = st.columns(2)
        with chart_cols[0]:
            st.subheader("Stav energie")
            st.altair_chart(
                build_line_chart(chart_source_df, "spotreba_energie", "Stav energie", ENERGY_STATE_COLOR),
                width="stretch",
            )
        with chart_cols[1]:
            st.subheader("Spotřeba energie")
            actual_chart = build_bar_chart(
                chart_source_df,
                "spotreba",
                "Spotřeba energie",
                ENERGY_CONSUMPTION_COLOR,
            )
            st.altair_chart(
                (
    ...
    chart_cols = st.columns(2)
    with chart_cols[0]:
        st.subheader(f"Spotřeba energie - {detail_level.lower()}")
...
def render_data_table(df: pd.DataFrame, detail_df: pd.DataFrame, detail_level: str) -> None:
    if detail_level == "Ne" or detail_df.empty:
        ...
        table_df = format_consumption_dataframe(
            table_df,
            columns=("Stav energie", "Spotřeba energie", "Kumulovaná spotřeba"),
        )
...
def render_export_button(df: pd.DataFrame, selected_ident: str, start_date: datetime.date, end_date: datetime.date, detail_level: str) -> None:
    ...
    file_name = f"spotreba_energie_{selected_ident}_{start_date}_{end_date}_{file_suffix}.xlsx"
    excel_bytes = dataframe_to_excel_bytes(build_export_dataframe(df), "Spotreba energie")
    st.download_button(
...
def render_dashboard() -> None:
    ...
    if not st.session_state.get(APPLIED_KEY):
        st.info("Klikněte na `Načíst data` pro zobrazení dat vybraného kalorimetru.")
        return
    ...
    if measurements_df.empty:
        ...
        st.title(f"Spotřeba energie - {selected_ident}")
    ...
    with st.container(border=True):
        ...
        boundary_display_df = format_consumption_dataframe(boundary_table, columns=("Stav energie",)).set_index("Datum")
    ...
    if not change_table.empty:
        with st.container(border=True):
            st.subheader("Resety nebo výměny kalorimetrů")
            st.table(format_consumption_dataframe(change_table, columns=("Stav energie",)))
