...
st.set_page_config(
    page_title="Review outlieru vodomeru",
    page_icon="🔎",
    layout="wide",
)


require_page_access("vodomery_outlier_review")
...
def load_reviews(
    ...
    if not access_token:
        raise DashboardApiError("Chybi bearer token pro dashboard API.")
    return list_vodomery_outlier_reviews(
        access_token,
...
def render_page() -> None:
    st.title("Review outlieru vodomeru")
    ...
    with filter_col:
        with st.form("vodomery_outlier_review_filters"):
    ...
    for row in reviews:
        ...
        with st.expander(expander_title, expanded=row["review_status"] == "PENDING"):
            ...
            with st.form(f"vodomery_outlier_review_form_{review_id}"):
...
try:
    ...
    st.error("Nepodarilo se nacist review outlieru vodomeru.")
    st.exception(exc)
