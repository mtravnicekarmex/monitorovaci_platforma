...
def render_web_search_page() -> None:
    with st.container():
            ...
            if st.button("Uložit monitor"):
                if not (url and vyrazy and email):
                    ...
                    if not normalized_url:
                        ...
                        if exist_monitor:
                            changed_identity = exist_monitor.url != normalized_url or exist_monitor.email != clean_email
                            exist_monitor.url = normalized_url
                            exist_monitor.email = clean_email
                            exist_vyrazy = json.loads(exist_monitor.vyrazy)
                            nove_vyrazy = [vyraz for vyraz in vyrazy_list if vyraz not in exist_vyrazy]
                            if not nove_vyrazy:
                                if changed_identity:
                                    session.commit()
                                ...
                                session.commit()
                                st.success(
                                    f"Aktualizován monitor, přidány nové výrazy: {', '.join(nove_vyrazy)}"
                                )
                        else:
                            monitor = Monitor(
                                url=normalized_url,
