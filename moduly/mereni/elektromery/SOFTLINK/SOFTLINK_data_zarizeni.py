...
class SoftlinkDeviceAuthError(RuntimeError):
...
def _build_device_headers(auth_token: str | None = None) -> dict[str, str]:
...
def _wait_for_auth_token(page, *, timeout_ms: int) -> str:
    ...
    if last_error is not None:
        raise last_error
    raise SoftlinkDeviceAuthError("SOFTLINK login nevratil autorizacni token pro seznam zarizeni.")


def _fetch_devices_with_request_context(
    context,
    ...
    *,
    ...
    response = context.request.post(
        SOFTLINK_DEVICE_API_URL,
        headers=_build_device_headers(auth_token),
        data={
            "mit_id": 105,
...
def _raise_for_invalid_device_response(response: object) -> None:
    if not isinstance(response, dict):
        raise RuntimeError("SOFTLINK seznam zarizeni nevratil ocekavanou odpoved.")

    status = int(response.get("status") or 0)
    if status in (401, 403):
        raise SoftlinkDeviceAuthError("SOFTLINK session/login neautorizovany pro seznam zarizeni.")
    if status != 200:
        raise RuntimeError(f"SOFTLINK seznam zarizeni vratil neocekavany HTTP status {status}.")
    if not isinstance(response.get("data"), list):
        raise RuntimeError("SOFTLINK seznam zarizeni nevratil ocekavany seznam dat.")
...
def _wait_for_authenticated_device_fetch(
    context,
    request_window: _SoftlinkRequestWindow,
    *,
    timeout_ms: int,
    ...
    while monotonic() < deadline:
        try:
            last_response = _fetch_devices_with_request_context(
                context,
                request_window,
    ...
    if last_response is not None:
        _raise_for_invalid_device_response(last_response)
    if last_error is not None:
        raise last_error
...
def _try_fetch_with_saved_session(*, headless: bool, timeout_ms: int):
    ...
    with sync_playwright() as playwright:
        ...
        try:
            response = _fetch_devices_with_request_context(
                context,
                request_window,
                auth_token=auth_token,
            )
...
def _login_and_fetch(*, headless: bool, timeout_ms: int):
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            response = _wait_for_authenticated_device_fetch(
                context,
                request_window,
