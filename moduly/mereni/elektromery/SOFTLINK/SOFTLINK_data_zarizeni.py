    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            with page.expect_popup(timeout=timeout_ms) as popup_info:
                page.get_by_role("link", name="Vstoupit do portálu").click(timeout=timeout_ms)
            ...
            portal.get_by_role("textbox", name="Přístupové jméno").fill(USERNAME, timeout=timeout_ms)
            portal.get_by_role("textbox", name="Přístupové heslo").fill(PASSWORD, timeout=timeout_ms)
            portal.get_by_role("button", name="Přihlásit").click(timeout=timeout_ms)
            portal.wait_for_selector("text=Odhlásit", timeout=timeout_ms)

            context.storage_state(path=str(auth_path))
            response = _fetch_devices_from_page(portal, request_window)
            portal.wait_for_timeout(1500)
            return response
        finally:
            browser.close()
