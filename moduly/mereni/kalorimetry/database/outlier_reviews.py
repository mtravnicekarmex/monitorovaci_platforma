...
from moduly.mereni.kalorimetry.database.models import KalorimetryOutlierReview
...
def ensure_kalorimetry_outlier_review_table() -> None:
    with ENGINE_PG.begin() as conn:
        conn.execute(text("CREATE SCHEMA IF NOT EXISTS monitoring"))
        conn.execute(
            text(
                """
                CREATE TABLE IF NOT EXISTS monitoring.kalorimetry_outlier_reviews (
                    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                    identifikace VARCHAR(250) NOT NULL,
                    date TIMESTAMP WITHOUT TIME ZONE NOT NULL,
                    zdroj VARCHAR(20) NOT NULL,
                    source_recid BIGINT,
                    seriove_cislo VARCHAR(100) NOT NULL,
                    interval_minutes INTEGER NOT NULL,
                    detection_kind VARCHAR(30) NOT NULL,
                    current_objem DOUBLE PRECISION NOT NULL,
                    baseline_objem DOUBLE PRECISION,
                    baseline_date TIMESTAMP WITHOUT TIME ZONE,
                    candidate_delta DOUBLE PRECISION NOT NULL,
                    threshold_delta DOUBLE PRECISION,
                    sample_size INTEGER,
                    median_delta DOUBLE PRECISION,
                    p90_delta DOUBLE PRECISION,
                    p99_delta DOUBLE PRECISION,
                    ...
                    CONSTRAINT ck_kalorimetry_outlier_review_detection_kind_valid
                        CHECK (detection_kind IN ('NORMAL_DELTA','GAP_MEAN')),
                    CONSTRAINT ck_kalorimetry_outlier_review_status_valid
                        CHECK (review_status IN ('PENDING','CONFIRMED_OUTLIER','CONFIRMED_CONSUMPTION')),
                    CONSTRAINT uq_kalorimetry_outlier_review_ident_date_source
                        UNIQUE (identifikace, date, zdroj)
                )
                """
            )
        )
        conn.execute(
            text(
                """
                CREATE INDEX IF NOT EXISTS ix_kalorimetry_outlier_review_status_date
                ON monitoring.kalorimetry_outlier_reviews (review_status, date)
                """
            )
        )
        conn.execute(
            text(
                """
                CREATE INDEX IF NOT EXISTS ix_kalorimetry_outlier_review_ident_date
                ON monitoring.kalorimetry_outlier_reviews (identifikace, date)
                """
            )
        )
...
def build_outlier_review_key(row: dict[str, object] | KalorimetryOutlierReview) -> tuple[str, object, str]:
    if isinstance(row, KalorimetryOutlierReview):
        return (
...
def serialize_review_row(row: KalorimetryOutlierReview) -> dict[str, object]:
    return {
        "id": row.id,
    ...
    *,
    ...
    try:
        rows = db_session.execute(
            select(KalorimetryOutlierReview).where(
                tuple_(
                    KalorimetryOutlierReview.identifikace,
                    KalorimetryOutlierReview.date,
                    KalorimetryOutlierReview.zdroj,
                ).in_(unique_keys)
            )
        ).scalars().all()
        return {
...
def upsert_outlier_review_candidates(
    ...
    *,
    ...
    db_session = session or Session(ENGINE_PG, autoflush=False, expire_on_commit=False)
    try:
        insert_stmt = insert(KalorimetryOutlierReview).values(rows)
...
def list_outlier_reviews(
    *,
    ...
    with Session(ENGINE_PG, autoflush=False, expire_on_commit=False) as session:
        query = select(KalorimetryOutlierReview)

        if review_status:
            query = query.where(
                KalorimetryOutlierReview.review_status == normalize_review_status(review_status)
            )
        if resolved_ident:
            query = query.where(KalorimetryOutlierReview.identifikace == resolved_ident)
        if resolved_source != "VSE":
            query = query.where(KalorimetryOutlierReview.zdroj == resolved_source)

        rows = session.execute(
            query.order_by(
                KalorimetryOutlierReview.date.desc(),
                KalorimetryOutlierReview.id.desc(),
...
def update_outlier_review(
    ...
    *,
    ...
    with Session(ENGINE_PG, autoflush=False, expire_on_commit=False) as session:
        row = session.get(KalorimetryOutlierReview, int(review_id))
        if row is None:
