...
def ensure_plynomery_outlier_email_delivery_table() -> None:
    ensure_plynomery_alerting_tables()
    with ENGINE_PG.begin() as conn:
        conn.execute(text("CREATE SCHEMA IF NOT EXISTS monitoring"))
        PlynomeryOutlierEmailDelivery.__table__.create(bind=conn, checkfirst=True)
    ...
    with Session(ENGINE_PG, autoflush=False, expire_on_commit=False) as session:
        reviews = (
            session.execute(
                ...
                .where(PlynomeryOutlierReview.id.in_(normalized_review_ids))
                .order_by(PlynomeryOutlierReview.date.asc(), PlynomeryOutlierReview.id.asc())
            )
            .scalars()
            .all()
        )
        ...
        rules = (
            session.execute(
                select(PlynomeryAlertRule)
                .where(PlynomeryAlertRule.enabled.is_(True))
                .order_by(PlynomeryAlertRule.recipient_email.asc(), PlynomeryAlertRule.rule_name.asc())
            )
            .scalars()
            .all()
        )
        if not rules:
            logger.info("No enabled plynomery alert rules configured for outlier notifications.")
            return {
                "matched": 0,
                "emails_sent": 0,
                "deliveries_sent": 0,
        ...
        for recipient_email, items in grouped_candidates.items():
            ...
            try:
                ...
                for item in items:
                    ...
                    if isinstance(delivery, PlynomeryOutlierEmailDelivery):
...
def _migrate_outlier_email_delivery_table(conn) -> None:
    ...
    if "plynomery_outlier_email_deliveries" not in inspector.get_table_names(schema="monitoring"):
        return

    columns = {column["name"] for column in inspector.get_columns("plynomery_outlier_email_deliveries", schema="monitoring")}
    if "rule_id" not in columns:
        conn.execute(text("ALTER TABLE monitoring.plynomery_outlier_email_deliveries ADD COLUMN rule_id INTEGER"))
    if "severity" not in columns:
        conn.execute(
            text(
                "ALTER TABLE monitoring.plynomery_outlier_email_deliveries "
    ...
    conn.execute(
        text(
            "ALTER TABLE monitoring.plynomery_outlier_email_deliveries "
            "DROP CONSTRAINT IF EXISTS uq_plynomery_outlier_email_delivery_review_recipient"
        )
    )
    conn.execute(
        text(
            "ALTER TABLE monitoring.plynomery_outlier_email_deliveries "
            "DROP CONSTRAINT IF EXISTS uq_plynomery_outlier_email_delivery_review_rule_recipient"
        )
    )
    conn.execute(
        text(
            "ALTER TABLE monitoring.plynomery_outlier_email_deliveries "
            "ADD CONSTRAINT uq_plynomery_outlier_email_delivery_review_rule_recipient "
...
def _load_existing_deliveries(
    session: Session,
    review_ids: list[int],
) -> dict[tuple[int, int | None, str], PlynomeryOutlierEmailDelivery]:
    rows = session.execute(
        select(PlynomeryOutlierEmailDelivery).where(PlynomeryOutlierEmailDelivery.review_id.in_(review_ids))
    ).scalars().all()
    return {
        (
            int(row.review_id),
...
def _build_candidate(
    *,
    rule: PlynomeryAlertRule,
    review: PlynomeryOutlierReview,
...
def _compute_outlier_severity(review: PlynomeryOutlierReview) -> str:
...
def _build_subject(items: list[dict[str, object]]) -> str:
    return f"[Plynomery] Nove outliery k review ({len(items)})"
...
def _build_html_body(items: list[dict[str, object]]) -> str:
    ...
    return (
        "<html><body>"
        f"<p>Byly nalezeny nove outliery plynomeru k manualnimu review. Generovano {escape(generated_at)}.</p>"
        "<p>Zkontrolujte prosim dashboard stranku Plynomery Outlier Review.</p>"
