        ...
        for recipient_email, items in grouped_candidates.items():
            ...
            try:
                ...
                for item in items:
                    ...
                    if isinstance(delivery, PlynomeryAlertDelivery):
...
def _load_events(session: Session, event_ids: list[int], *, active_only: bool) -> list[PlynomeryAnomalyEvent]:
    if not event_ids:
        return []

    query = select(PlynomeryAnomalyEvent).where(PlynomeryAnomalyEvent.id.in_(event_ids))
    if active_only:
        query = query.where(PlynomeryAnomalyEvent.is_active.is_(True))
    else:
        query = query.where(PlynomeryAnomalyEvent.resolved.is_(True))
...
def _load_existing_deliveries(
    session: Session,
    event_ids: list[int],
) -> dict[tuple[int, int | None, str, str], PlynomeryAlertDelivery]:
    rows = session.execute(
        select(PlynomeryAlertDelivery).where(PlynomeryAlertDelivery.event_id.in_(event_ids))
    ).scalars().all()
    return {
        (
            int(row.event_id),
...
def _rule_matches_event(*, rule: PlynomeryAlertRule, event: PlynomeryAnomalyEvent, alert_state: str) -> bool:
    if not rule.enabled:
        return False
...
def _build_subject(items: list[dict[str, object]]) -> str:
    return f"[Plynomery] Souhrn alertu ({len(items)})"
...
def _build_html_body(items: list[dict[str, object]]) -> str:
    ...
    return (
        "<html><body>"
        f"<p>Souhrn alertu plynomeru vygenerovany {escape(now_label)}.</p>"
