...
from moduly.mereni.plynomery.database.models import PlynomeryOutlierReview
...
def build_outlier_review_key(row: dict[str, object] | PlynomeryOutlierReview) -> tuple[str, object, str]:
    if isinstance(row, PlynomeryOutlierReview):
        return (
...
def serialize_review_row(row: PlynomeryOutlierReview) -> dict[str, object]:
    return {
        "id": row.id,
    ...
    *,
    ...
    try:
        rows = db_session.execute(
            select(PlynomeryOutlierReview).where(
                tuple_(
                    PlynomeryOutlierReview.identifikace,
                    PlynomeryOutlierReview.date,
                    PlynomeryOutlierReview.zdroj,
                ).in_(unique_keys)
            )
        ).scalars().all()
        return {
...
def upsert_outlier_review_candidates(
    ...
    *,
    ...
    db_session = session or Session(ENGINE_PG, autoflush=False, expire_on_commit=False)
    try:
        insert_stmt = insert(PlynomeryOutlierReview).values(rows)
...
def list_outlier_reviews(
    *,
    ...
    with Session(ENGINE_PG, autoflush=False, expire_on_commit=False) as session:
        query = select(PlynomeryOutlierReview)

        if review_status:
            query = query.where(
                PlynomeryOutlierReview.review_status == normalize_review_status(review_status)
            )
        if resolved_ident:
            query = query.where(PlynomeryOutlierReview.identifikace == resolved_ident)
        if resolved_source != "VSE":
            query = query.where(PlynomeryOutlierReview.zdroj == resolved_source)

        rows = session.execute(
            query.order_by(
                PlynomeryOutlierReview.date.desc(),
                PlynomeryOutlierReview.id.desc(),
...
def update_outlier_review(
    ...
    *,
    ...
    with Session(ENGINE_PG, autoflush=False, expire_on_commit=False) as session:
        row = session.get(PlynomeryOutlierReview, int(review_id))
        if row is None:
