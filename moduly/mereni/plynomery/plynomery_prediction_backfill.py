...
BACKFILL_DEFAULT_START = datetime(2026, 4, 21)
BACKFILL_DEFAULT_HISTORY_START = datetime(2025, 11, 1)
BACKFILL_DEFAULT_ARCHIVE_VERSION = 1
BACKFILL_HISTORY_MONTHS_REQUIRED = 3
PLYNOMERY_BACKFILL_MODEL_VERSIONS = (1, 2)


@dataclass(frozen=True)
class PlynomeryBackfillIdentifierHistory:
    identifier: str
...
@dataclass(frozen=True)
class PlynomeryBackfillPlanItem:
    identifier: str
...
@dataclass(frozen=True)
class PlynomeryBackfillPlan:
    start_date: datetime
    end_date: datetime
    archive_version: int
    model_versions: tuple[int, ...]
    items: tuple[PlynomeryBackfillPlanItem, ...]
...
@dataclass(frozen=True)
class PlynomeryBackfillDryRunWeekResult:
    forecast_period: PredictionForecastPeriod
...
@dataclass(frozen=True)
class PlynomeryBackfillDryRunResult:
    archive_run_id: str
    plan: PlynomeryBackfillPlan
    weeks: tuple[PlynomeryBackfillDryRunWeekResult, ...]
...
@dataclass(frozen=True)
class PlynomeryBackfillWriteWeekResult(PlynomeryBackfillDryRunWeekResult):
    inserted_selected_decision_count: int = 0
    inserted_candidate_metric_count: int = 0
    inserted_profile_snapshot_count: int = 0


@dataclass(frozen=True)
class PlynomeryBackfillWriteResult:
    archive_run_id: str
    plan: PlynomeryBackfillPlan
    weeks: tuple[PlynomeryBackfillWriteWeekResult, ...]
    ...
    @property
    def inserted_selected_decision_count(self) -> int:
        return sum(week.inserted_selected_decision_count for week in self.weeks)
...
@dataclass(frozen=True)
class PlynomeryBackfillVerifyProfileSource:
    archive_source: str
...
@dataclass(frozen=True)
class PlynomeryBackfillVerifyCandidateMetrics:
    archive_version: int
...
@dataclass(frozen=True)
class PlynomeryBackfillVerifyDecisions:
    decision_count: int
    identifier_count: int
    forecast_week_count: int
    identifier_week_count: int
...
@dataclass(frozen=True)
class PlynomeryBackfillVerifyResult:
    start_date: datetime
    end_date: datetime
    archive_version: int
    profile_sources: tuple[PlynomeryBackfillVerifyProfileSource, ...]
    decisions: PlynomeryBackfillVerifyDecisions
    candidate_metrics: PlynomeryBackfillVerifyCandidateMetrics
...
class _BackfillWeekCalculation:
    summary: PlynomeryBackfillDryRunWeekResult
...
def load_plynomery_backfill_identifier_history(
    session,
    *,
    start_date: datetime = BACKFILL_DEFAULT_START,
    identifiers: Sequence[str] | None = None,
) -> tuple[PlynomeryBackfillIdentifierHistory, ...]:
    ...
    return tuple(
        PlynomeryBackfillIdentifierHistory(
...
def build_plynomery_backfill_plan(
    histories: Iterable[PlynomeryBackfillIdentifierHistory],
    *,
    start_date: datetime = BACKFILL_DEFAULT_START,
    end_date: datetime,
    archive_version: int = BACKFILL_DEFAULT_ARCHIVE_VERSION,
    model_versions: Sequence[int] = PLYNOMERY_BACKFILL_MODEL_VERSIONS,
    existing_weekly_rebuild_periods: Iterable[tuple[str, datetime]] = (),
    max_identifiers: int | None = None,
    max_weeks: int | None = None,
) -> PlynomeryBackfillPlan:
    ...
    items: list[PlynomeryBackfillPlanItem] = []
    ...
    global_first_week = floor_calendar_week_start(start_date)
...
def plan_plynomery_prediction_backfill(
    *,
    start_date: datetime = BACKFILL_DEFAULT_START,
    end_date: datetime,
    ...
    resolved_history_start_date = (
        history_start_date or BACKFILL_DEFAULT_HISTORY_START
    )
    ...
    try:
        histories = load_plynomery_backfill_identifier_history(
            session,
            start_date=resolved_history_start_date,
            identifiers=identifiers,
        )
        return build_plynomery_backfill_plan(
            histories,
            start_date=start_date,
            end_date=end_date,
            archive_version=archive_version,
...
def verify_plynomery_prediction_backfill(
    *,
    start_date: datetime = BACKFILL_DEFAULT_START,
    end_date: datetime,
    archive_version: int = BACKFILL_DEFAULT_ARCHIVE_VERSION,
    ...
    try:
        ...
        table_state = (
            session.execute(
                text(
                    """
                    SELECT
                        to_regclass('monitoring.prediction_selected_model_snapshots')
                            AS decision_table,
        ...
        missing_tables = []
        if table_state["decision_table"] is None:
                ...
                session.execute(
                    text(
                        ...
                        SELECT
                            ...
                            COUNT(DISTINCT forecast_period_start)
                                AS forecast_week_count,
                        ...
                        WHERE
                            ...
                            AND forecast_period_end > :start_date
                            {identifier_filter}
                        """
                    ),
                    params,
        ...
        return PlynomeryBackfillVerifyResult(
            start_date=start_date,
            end_date=end_date,
            archive_version=archive_version,
            profile_sources=tuple(
                PlynomeryBackfillVerifyProfileSource(
            ...
            decisions=PlynomeryBackfillVerifyDecisions(
                ...
                identifier_count=int(decision_row["identifier_count"] or 0),
                forecast_week_count=int(
                    decision_row["forecast_week_count"] or 0
                ),
                identifier_week_count=int(
                    decision_row["identifier_week_count"] or 0
                ),
            ),
            candidate_metrics=PlynomeryBackfillVerifyCandidateMetrics(
                archive_version=archive_version,
...
def dry_run_plynomery_prediction_backfill(
    plan: PlynomeryBackfillPlan,
    *,
    archive_run_id: str,
    session=None,
) -> PlynomeryBackfillDryRunResult:
    ...
    try:
        ...
        for forecast_start, items in _group_plan_items_by_week(plan):
            try:
                ...
                session.rollback()
        return PlynomeryBackfillDryRunResult(
            archive_run_id=archive_run_id,
            plan=plan,
...
def write_plynomery_prediction_backfill(
    plan: PlynomeryBackfillPlan,
    *,
    archive_run_id: str,
    session=None,
) -> PlynomeryBackfillWriteResult:
    if not archive_run_id:
    ...
    if session is None:
        ...
        if lock_handle is None:
            raise RuntimeError(
                "Plynomery backfill write stopped: quarter_hour_job lock is busy."
            )
        try:
            session = get_session_pg()
        except Exception:
    ...
    try:
        ...
        for forecast_start, items in _group_plan_items_by_week(plan):
            try:
                calculation = _calculate_plynomery_backfill_week(
                    session,
                    forecast_start=forecast_start,
                    items=items,
                    archive_run_id=archive_run_id,
                ...
                session.rollback()
                inserted_selected_decision_count = (
                    persist_selected_model_decisions(
                        session,
                        calculation.selected_decisions,
                ...
                session.commit()
                week_results.append(
                    PlynomeryBackfillWriteWeekResult(
        ...
        return PlynomeryBackfillWriteResult(
            archive_run_id=archive_run_id,
            plan=plan,
            weeks=tuple(week_results),
        )
...
def _group_plan_items_by_week(
    plan: PlynomeryBackfillPlan,
) -> tuple[tuple[datetime, tuple[PlynomeryBackfillPlanItem, ...]], ...]:
    grouped: dict[datetime, list[PlynomeryBackfillPlanItem]] = {}
...
def _calculate_plynomery_backfill_week(
    session,
    *,
    forecast_start: datetime,
    items: Sequence[PlynomeryBackfillPlanItem],
    archive_run_id: str,
    archive_version: int,
    ...
    for model_version in model_versions:
        definition = plynomery_prediction._get_candidate_model_definition(model_version)
        if definition is None:
            raise ValueError(f"Unknown plynomery candidate model version: {model_version}")
        windows = plynomery_prediction._build_windows_for_definition(
            definition,
            reference_time=forecast_period.start,
        )
        summary = plynomery_prediction._rebuild_candidate_model(
            session,
        ...
        rolling_result = (
            plynomery_prediction._run_candidate_rolling_weekly_backtest_with_devices(
                session,
                definition=definition,
        ...
        summaries.append(
            plynomery_prediction._summary_with_rolling_backtest(
                summary,
                fold_count=plynomery_prediction.MODEL_ROLLING_BACKTEST_FOLD_COUNT,
    ...
    selected_summary = plynomery_prediction.select_best_model_summary(summaries)
    if selected_summary is None:
        selected_summary = summaries[0]

    deployable_profile_catalog = (
        plynomery_prediction._load_deployable_profile_catalog(
            session,
            device_summaries,
        )
    )
    selected_decisions = (
        plynomery_prediction._build_dry_run_selected_model_decisions(
    ...
    if include_profile_snapshot_rows:
        if profile_windows is None:
            raise RuntimeError("Plynomery backfill has no candidate windows.")
        selected_profile_snapshot_rows = (
    ...
    selected_profile_pair_count = (
        plynomery_prediction._count_profile_snapshot_pairs(
            selected_profile_snapshot_rows,
        )
        if include_profile_snapshot_rows
...
def _build_backfill_candidate_metric_rows(
    ...
    *,
    ...
    for summary in device_summaries:
        ...
        rows.append(
            {
                "medium_key": plynomery_prediction.PLYNOMERY_MEDIUM_KEY,
                ...
                "model_key": plynomery_prediction._device_model_key(summary),
...
def _rank_device_summaries_by_policy(
    ...
    for summary in device_summaries:
        if (
            ...
            and plynomery_prediction._device_summary_has_fallback_metrics(summary)
            and summary.rolling_wape is not None
            and summary.rolling_backtest_fold_count
            >= plynomery_prediction.MODEL_SELECTION_MIN_FOLD_COUNT
            and summary.rolling_coverage >= plynomery_prediction.MODEL_SELECTION_COVERAGE_THRESHOLD
        ):
    ...
    for identifikace, summaries in grouped.items():
        for rank, summary in enumerate(
            sorted(summaries, key=plynomery_prediction._device_summary_selection_key),
            start=1,
        ):
