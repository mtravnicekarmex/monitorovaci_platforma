    ...
    *,
    ...
    with Session(ENGINE_PG, autoflush=False, expire_on_commit=False) as session:
        review_row = session.execute(
            ...
            .where(VodomeryOutlierReview.id == int(review_id))
            .with_for_update()
        ).scalar_one_or_none()
        ...
        if previous_status != resolved_status:
            ...
            logger.info(
                "Applied outlier review rebuild | review_id=%s | identifikace=%s | zdroj=%s | status=%s | summary=%s",
                review_row.id,
...
def _rebuild_after_review_update(session: Session, review_row: VodomeryOutlierReview) -> dict[str, object]:
...
def _load_review_overrides(
    ...
    *,
    ...
    rows = session.execute(
        select(VodomeryOutlierReview.date, VodomeryOutlierReview.review_status)
        .where(
            VodomeryOutlierReview.identifikace == identifikace,
            VodomeryOutlierReview.zdroj == zdroj,
            VodomeryOutlierReview.date >= start_date,
            VodomeryOutlierReview.review_status.in_(
...
def _load_actual_measurements(
    ...
    *,
    ...
    zdroj: str,
    start_date,
) -> list[Mereni_vodomery]:
    return session.execute(
        select(Mereni_vodomery)
        .where(
            Mereni_vodomery.identifikace == identifikace,
            Mereni_vodomery.zdroj == zdroj,
            Mereni_vodomery.synthetic.is_(False),
            Mereni_vodomery.date >= start_date,
        )
        .order_by(Mereni_vodomery.date.asc(), Mereni_vodomery.id.asc())
    ).scalars().all()


def _rebuild_measurements_for_review(session: Session, review_row: VodomeryOutlierReview) -> dict[str, object]:
    ...
    session.execute(
        delete(VodomeryOutlierReview).where(
            VodomeryOutlierReview.identifikace == identifikace,
            VodomeryOutlierReview.zdroj == zdroj,
            VodomeryOutlierReview.date >= start_date,
            VodomeryOutlierReview.review_status == "PENDING",
            VodomeryOutlierReview.id != review_row.id,
        )
    )
    ...
    *,
    ...
    profiles = session.execute(
        select(VodomeryProfilesAnomaly).where(
            ...
            VodomeryProfilesAnomaly.identifikace == identifikace,
        )
    ).scalars().all()
    if not profiles:
        return {
    ...
    *,
    ...
    if event_ids:
        for event_id_chunk in chunked(event_ids):
            session.execute(
                delete(VodomeryAlertDelivery).where(
                    VodomeryAlertDelivery.event_id.in_(event_id_chunk)
                )
            )

    session.execute(
        delete(VodomeryAnomalyEvent).where(
            VodomeryAnomalyEvent.identifikace == identifikace,
            VodomeryAnomalyEvent.model_version == model_version,
        )
    )
    session.execute(
        delete(VodomeryEventState).where(
            VodomeryEventState.identifikace == identifikace,
    ...
    scores = session.execute(
        select(VodomeryAnomalyScore)
        .where(
            VodomeryAnomalyScore.identifikace == identifikace,
            VodomeryAnomalyScore.model_version == model_version,
        )
        .order_by(VodomeryAnomalyScore.id.asc())
    ).scalars().all()

    expected_zero = bool(
        session.execute(
            select(VodomeryExpectedZero.identifikace).where(
                VodomeryExpectedZero.identifikace == identifikace
            )
        ).scalar_one_or_none()
    )
