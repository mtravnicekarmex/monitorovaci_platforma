...
from moduly.mereni.plynomery.plynomery_prediction_backfill import (
    BACKFILL_DEFAULT_ARCHIVE_VERSION,
    BACKFILL_DEFAULT_HISTORY_START,
    BACKFILL_DEFAULT_START,
    dry_run_plynomery_prediction_backfill,
    plan_plynomery_prediction_backfill,
    verify_plynomery_prediction_backfill,
    write_plynomery_prediction_backfill,
)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Plan or dry-run historical plynomery prediction backfill. "
...
def _build_plan(args: argparse.Namespace):
    return plan_plynomery_prediction_backfill(
...
def build_write_report(result) -> dict[str, Any]:
    return {
        ...
        "inserted_selected_decision_count": (
            result.inserted_selected_decision_count
        ),
        ...
        "weeks": [
            {
                ...
                "selected_profile_pair_count": week.selected_profile_pair_count,
                "inserted_selected_decision_count": (
                    week.inserted_selected_decision_count
                ),
...
def build_verify_report(result) -> dict[str, Any]:
    return {
        ...
        "decisions": {
            "decision_count": result.decisions.decision_count,
            "identifier_count": result.decisions.identifier_count,
            "forecast_week_count": result.decisions.forecast_week_count,
            "identifier_week_count": result.decisions.identifier_week_count,
        },
        "profile_sources": [
            {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "plan":
        ...
        plan = _build_plan(args)
        dry_run = dry_run_plynomery_prediction_backfill(
            plan,
        ...
        plan = _build_plan(args)
        write_result = write_plynomery_prediction_backfill(
            plan,
            archive_run_id=args.archive_run_id,
        )
        report = build_write_report(write_result)
    elif args.command == "verify":
        verify_result = verify_plynomery_prediction_backfill(
            start_date=args.start_date,
            end_date=args.end_date,
            archive_version=args.archive_version,
            identifiers=tuple(args.identifikace or ()),
