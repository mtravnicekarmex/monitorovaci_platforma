...
def test_build_post_write_refresh_minutes_offsets_scheduler_write_slots():
    assert SENSOR_DB_WRITE_MINUTES == (5, 16, 35, 47)
    assert build_post_write_refresh_minutes(SENSOR_DB_WRITE_MINUTES, buffer_minutes=1) == (6, 17, 36, 48)
    assert SENSOR_REFRESH_MINUTES == (6, 17, 36, 48)
...
def test_get_scheduler_job_run_minutes_reads_all_quarter_hour_job_slots():
    assert get_scheduler_job_run_minutes("quarter_hour_job") == (5, 16, 35, 47)
...
def test_build_interval_refresh_minutes_creates_regular_quarter_hour_slots_after_job_anchor():
    ...
    assert QUARTER_HOUR_PAGE_REFRESH_MINUTES == (6, 17, 36, 48)
...
def test_get_next_scheduled_refresh_skips_current_exact_slot():
    ...
    assert next_refresh == datetime.datetime(2026, 4, 8, 10, 48)
