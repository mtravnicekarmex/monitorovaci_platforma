...
from moduly.mereni.vodomery.alerting.service import _rule_matches_event
from moduly.mereni.vodomery.vodomery_events import EVENT_SUSTAINED_HIGH_USAGE
...
def test_rule_matches_event_duration_threshold_is_inclusive():
    rule = _rule(min_duration_minutes=60)

    assert not _rule_matches_event(
        rule=rule,
        event=_event(duration_minutes=59),
        alert_state="ACTIVE_THRESHOLD",
    )
    assert _rule_matches_event(
        rule=rule,
        event=_event(duration_minutes=60),
        alert_state="ACTIVE_THRESHOLD",
    )
